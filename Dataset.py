...
while True:
    ...
    if not ret:                # Handle the case where the video stream ends or the frame cannot be read.
        break
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)  # Converting colourful images to black and white images because cascade classifier uses gray scale images for detecting the faces.
...
if 'names.pkl' not in os.listdir('Data/'):      # If the pickle file is not present in our os we are going to create a new folder
    ...
    with open('Data/names.pkl', 'wb') as f:     # wb: writing format
        pickle.dump(names, f)
else:                                           # this function is used for loading all the names in the pickle file of our dataset
...
else:                                               # if the face_data pickle file is already present in the Data folder we are going to update or load the face_data.pkl file
